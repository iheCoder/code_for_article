...
Go 的标准 `net/http` 服务器为每个传入的连接生成一个新的 Goroutine 。这个模型之所以具有高度的可扩展性，完全得益于 Go 的调度器。Goroutine 与操作系统线程相比极其轻量 。当一个 Goroutine 因网络 I/O 而阻塞时，Go 调度器会将其交给集成的网络轮询器（netpoller）处理，并让底层的操作系统线程（M）去运行其他就绪的 Goroutine。这种机制避免了 I/O 阻塞整个线程的执行，从而能够在少量线程上实现大规模的并发 。这个调度模型通常被称为 G-M-P 模型，其中 G 代表 Goroutine，M 代表机器（OS 线程），P 代表处理器（逻辑上下文）27。
