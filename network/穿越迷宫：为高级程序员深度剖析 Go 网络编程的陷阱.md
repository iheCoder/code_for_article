...
2. **必须耗尽 Body**：如果不需要响应体的内容，仅仅关闭它是不够的。必须将其完全读出并丢弃，这样连接才有资格被复用。标准做法是在关闭之前调用 `io.Copy(io.Discard, resp.Body)` 。这个操作会排空 TCP 接收缓冲区，满足 HTTP 协议的要求，从而允许
...
全局的 `http.DefaultClient` 以及包级别的便捷函数如 `http.Get`，其内部使用的客户端*没有任何超时配置* 。



**后果**

一个缓慢或无响应的下游服务，就可能导致应用程序的 Goroutine 无限期地挂起，持续消耗资源，直到整个应用停止响应。这是生产环境中频繁出现的故障原因之一。正如 Cloudflare 的一篇博文所警告的，使用这些便捷函数将导致连接泄漏和文件描述符耗尽 。
...
- **`MaxIdleConnsPerHost` (默认值: )**: 对于需要向同一 API 端点发起大量并发请求的服务来说，这个值通常太低。低值会导致连接抖动（churning）：连接被频繁地建立和拆除，这不仅抵消了 Keep-Alive 带来的好处，还增加了 CPU 使用率和延迟 。
- **`MaxIdleConns`**: 这是跨所有主机的全局空闲连接限制。如果服务需要与许多不同的下游服务通信，这个限制可能会被触及，导致最旧的空闲连接被关闭，即使它们仍然有用 。
...
为了进行更精细的错误处理，可以检查一个错误是否实现了 `net.Error` 接口。这允许区分超时 (`netErr.Timeout()`) 和临时性网络问题 (`netErr.Temporary()`)，这些信息可以为重试逻辑提供依据 。
...
2.  **处理 `X-Forwarded-For`**：
    ...
    *   **信任边界**：你的信任边界是你自己控制的边缘基础设施（如负载均衡器）。配置你的边缘层，让它*覆盖*或*正确附加*客户端 IP 到 `X-Forwarded-For` 头部。应用代码应该只信任由这个边界添加的 IP。通常，这意味着取逗号分隔列表中的最后一个 IP 地址，因为它是离服务器最近、最可信的代理添加的。
...
2.  **等待现有请求完成**：等待所有正在处理的请求完成其生命周期。
3.  **关闭空闲连接**：关闭所有空闲的 Keep-Alive 连接。
4.  **超时退出**：通过传入的 `context` 设置一个最后期限，防止服务器因某个卡住的请求而无限期地等待，确保了关停过程的最终完成。

这种机制确保了服务的平滑过渡，避免了数据丢失和对客户端的突然中断，是现代云原生环境中服务部署和更新的必要条件。
