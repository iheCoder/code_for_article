...
-   $\sqrt{d_k}$: 这是**缩放因子 (Scale)**。$d_k$ 是 Key 向量的维度。为什么要除以它？想象一下，当向量维度很高时（比如 512），点积的结果可能会变得非常大，这会将 `Softmax` 函数推向其梯度很小的区域，导致学习过程不稳定。这就像在 Go 中处理一个数值巨大的 `float64`，如果不做归一化，后续的计算可能会溢出或失去精度。这是一个不起眼但至关重要的工程技巧。
...
-   **做什么**: 携带了位置信息的词向量，现在进入了 Transformer 的核心——一叠（比如 96 层）注意力模块。
...
-   **做什么**: 经过所有 Transformer 层的处理，模型最终会输出一个巨大的概率列表，预测下一个最可能的 Token 是什么。
-   **Go 类比**: 它不是总选择概率最高的那个词（否则回答会非常死板）。相反，它像是在玩一个“加权轮盘赌”。`temperature` 参数控制这个轮盘的随机性：温度高，轮盘更“平”，回答更有创意；温度低，轮盘更“陡”，回答更准确保守。`top-p` 则限制了只在最可能的一小部分词里玩轮盘赌。
