...
1. 多查询扩展：对用户查询做 *multi-query* 扩展。
   ...
   ，会先让模型生成一个针对该问题的“假设性”答案（伪查询文档），再用这个答案的向量去检索，能更精准地找到语义匹配的内容，有效降低召回盲点。

2. 混合检索：先按 `scope` 与元数据过滤（user_id、project_id、time_range、tags），再结合

    - 稀疏检索（BM25/keyword），
    - 稠密检索（embedding cosine/IP），
    - 时间/新鲜度因子（Recency Boost）。
