...
- 可变/较大类型用指针接收者；小且不可变用值接收者，利于栈上调用与内联。
- 含同步原语（如 `sync.Mutex`）的类型必须用指针，避免复制锁引发未定义行为。



Q4：返回切片/字符串会让底层大数组“被保留”吗？

答：可能会。返回的是“视图”，底层大数组会随它一起延长寿命。

- 场景示例：对 10MB 字节块做 `s := big[:100]` 并返回，会让 10MB 保持可达。
- 解决：若结果会长寿命，复制一份小切片再返回；字符串用 `strings.Clone` 或先转 `[]byte` 切片再构造新串。



Q5：channel 里传值还是传指针？

答：小 POD（纯数据、无指针）传值即可；切片/字符串/映射按值传只复制头部，通常也很好。

- 传指针的理由：对象很大且需要接收方原地修改；但要注意并发竞态与所有权约定。



Q6：map 的值用值类型还是指针？

答：小值/无指针优先用值，避免额外分配；大值或更新频繁用指针/索引以减少复制。

- 清空技巧：Go 1.21+ 可用 `clear(m)`；若需更积极释放内存，用 `make` 新 map 替换旧的。
...
- 使用具体类型或泛型，避免 `interface{}` 触发装箱；避免取局部变量地址；避免闭包捕获大对象。
- 让调用方传入可复用的缓冲区；用 `go build -gcflags=-m` 检查并迭代热函数。



Q8：`sync.Pool` 是否“Green Tea 友好”？

答：适合突发、短命对象，能削峰降分配，但不是缓存。

- GC 周期可能丢弃池内对象；`Put` 前务必重置内容，避免脏数据泄漏到下个请求。
- 不建议放巨型对象或持有外部大切片的对象，容易放大保留集。



Q9：`time.After` 会不会导致隐性分配或“泄漏”？

答：在循环中会频繁分配定时器且无法回收，容易堆积。

- 推荐写法：复用 `time.Timer` 并 `Reset`，并配对 `Stop`。
...
答：短期复用可 `s = s[:0]`；若生命周期变长或容量过大，应“缩容”。

- 做法：`t := append(make([]T, 0, need), s[:n]...)` 或 `copy` 到新小切片；字符串子串必要时显式克隆。



Q11：日志与 JSON 序列化，如何减少分配与 GC 压力？
...
Q12：能否利用“不可指针化”类型优化 GC？

答：可以。把某些 ID/标记用定长数组表示（如 `[16]byte`）。

- 这类值不含指针，GC 不会深入扫描；放在 `[]T` 中也利于批量扫描和缓存友好。



Q13：如何降低写屏障开销？

答：减少“指针写入次数”和“指针字段数量”。

- 设计上更偏向 `[]T` 而非 `[]*T`；预先 `make` 够容量，避免扩容搬迁时大量指针写；复用内部缓冲而非频繁替换。
...
- 常用热路径使用 `[]byte`/`[]T` 明确类型；避免 `any`/`interface{}`；必要时提供“高层易用 + 底层零分配”的双轨接口。
