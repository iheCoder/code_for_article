...
O(RcdotFP)（R是规则数，F是事实数，P是规则的平均模式数），这在现实世界中是不可接受的。
...
#### 4. FIFO (First-In, First-Out): “先到先得”的公平原则
...
1. **第一轮：按 Salience 排序**。优先级最高的规则胜出。
2. **第二轮：决胜局 (Tie-Breaker)**。如果多条规则的 Salience 相同，则会比较它们的 **Specificity**，更特殊的规则胜出。
3. **第三轮：终极决胜局**。如果连 Specificity 都完全一样，引擎会采用一个最终策略，比如 **LIFO**，来确保每次执行的顺序都是可预测的。

这种分层的组合策略，既给了开发者用 `Salience` 进行显式控制的权力，又通过 `Specificity` 和 `LIFO`
...
- **核心技术**：早期 Drools 使用 Rete 算法，但后来发展出了其改进版 **ReteOO** 和更先进的 **PHREAK** 算法。PHREAK
  算法在规则的惰性评估、更好的可维护性和并行计算方面做了优化，尤其是在规则库频繁动态更新的场景下表现更佳。
...
- **生态与工具**：Drools 提供了强大的**决策表（Decision Table）**，让业务人员可以在 Excel 中定义规则。它还提供了
...
* **实现要点**：
    * SaaS：LaunchDarkly / Firebase Remote Config；
    * 自建：Unleash + Redis，本地 SDK 带实时缓存，评估逻辑 O(1)；
    * 常见 DSL：`user.id % 100 < 5 AND app.version >= "3.2.0"`。

#### 2.3 「复杂关联」——**轻量策略服务 (YAML + DSL) / EasyRules**
...
* **实现要点**：
    * 在线推断之前先走规则 quick‑fail；
    * 推断输出再经规则 post‑filter（年龄、敏感词）。

> **混合范式**是多数大厂的终局：**“Flag + 轻量 DSL + Rete + ML”**分层协同，各取所长。
